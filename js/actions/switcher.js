    ...
    for (let index = 0; index < content_view.length; index++) {
        if (state === content_view[index].state ) {
            if (user_role === "admin" && state === "dashboard" )
               $.when(loadContent(state, content_view[index].title, content_view[index].links[0])).done(
                    function () {
                ...
                $.when(loadContent(state, content_view[index].title, content_view[index].links[1])).done(
                    function () {
                        //load dashboard datas
                    }
                )
            else if (user_role === "loan officer" && state === "dashboard")
                $.when(loadContent(state, content_view[index].title, content_view[index].links[2])).done(
                    function () {
                        //load dashboard datas
                    }
                )
            else
                $.when(loadContent(state, content_view[index].title, content_view[index].link)).done(
                    function () {
                        if (state === "users")
...
function loadContent(newState, title, urlPath) {
    $.ajax({
        ...
        error: function () {
            $("#mainContent").append("<h>" + title + " view could not be loaded </h1>"); //Displays an error message if content is not loaded;
        }
    });
}
