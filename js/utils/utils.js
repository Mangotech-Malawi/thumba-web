...
export function setRecordText(dataset, titleId, record) {
    if (dataset.clientType === "individual") {
        $(`#${titleId}`).text(
            `${dataset.recordFirstname} ${dataset.recordLastname} ${record}`
        );

    } else if (dataset.clientType === "organization") {
        $(`#${titleId}`).text(
